...
func buildTools() []anthropic.ToolUnionParam {
	return []anthropic.ToolUnionParam{
		...
		tool("subghz_rx_raw",
			"Start a raw Sub-GHz capture that is written to a .sub file on the SD card for protocol-level reverse engineering. Prefer this over subghz_receive when you need the raw timings (not just the decoded key). Xtreme firmware automatically appends the device arg.",
			props(
				reqProp("file", "string", "Destination .sub file path, e.g. /ext/subghz/raw_capture.sub"),
				optProp("frequency", "integer", "Frequency in Hz (required on some forks; defaults to firmware's last-used)"),
