...
var toolLevels = func() map[string]Level {
	...
	register(Low,
		...
		"audit_query", "audit_export", "audit_stats", "audit_verify",
		...
		"analyze_image",
		"list_apps",
		"ir_decode_file", "ir_universal_list", "ir_raw_decode", "ir_pronto_decode", "ir_pronto_encode", "ir_raw_encode",
		"rfid_raw_analyze",
		"onewire_search",
		"i2c_scan",
		...
		// nothing, so it is Low like the rest of the EMV decode family.
		"nfc_emv_tvr_decode",
		// TSI (tag 9B) bitfield decode: which functions the terminal
		// actually performed (offline-auth / CVM / risk-mgmt / issuer-auth
		...
		"nfc_iso15693_decode",
		// ISO 14443 Type B ATQB decode (ePassport / eID proximity
		...
		"cwt_decode",
		// Offline general COSE message decoder (RFC 9052 → type / headers /
		// payload / signature|tag|ciphertext). Pairs with cose_key_decode
		// and cwt_decode; structure only, no verification. v0.759.
		"cose_message_decode",
		...
		// parameters only, no crack/decrypt, no pdf2john hash emitted. Reads a
		// string, transmits nothing, so it is Low.
		"pdf_crack_triage",
		...
		// outer header only, no key derivation, no $keepass$ hash emitted. Reads
		// a string, transmits nothing, so it is Low.
		"kdbx_decode",
		...
		// transmits nothing, so it is Low.
		"mysql_password",
		// v0.505 — Domain Cached Credentials v2 (DCC2 / mscash2,
		// hashcat 2100) compute + verify: MD4(MD4(pw)+user) then
		// PBKDF2-HMAC-SHA1, the offline-crack-only cached domain logon
		// from a workstation SECURITY hive. Offline compute /
		// constant-time verify from operator-supplied strings — like
		// the other credential computes it derives a value and
		// transmits nothing, so it is Low.
		"dcc2",
		...
		"imsi_decode",
		// v0.550 (SIM-card identifier decode): iccid_decode — the
		// SIM-card leg completing the IMEI/IMSI/ICCID triad. 89 MII +
		// E.164 country code + Luhn check digit (the verification
		// anchor). Pure offline digit-field decode of a pasted number;
	...
	register(High,
		...
		"nfc_raw_frame", "nfc_apdu", "nfc_mfu_wrbl", "nfc_subcommand",
