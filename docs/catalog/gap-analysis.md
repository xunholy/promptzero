...
- `ir_raw_decode` (raw infrared timing → protocol) — ✅ shipped v0.413 — the IR analogue of `subghz_decode` and the complement to `ir_decode_file` (which only reads a .ir file's already-parsed entries). Decodes the NEC family (standard / extended / repeat) from a captured microsecond mark/space train, gated on NEC's address & command bitwise-inverse checksum, plus Sony SIRC (12/15/20-bit, v0.418) gated structurally on the 2400µs leader + exact bit count + per-bit timing (no confidently-wrong output); leader + every bit tolerance-matched. RC5/RC6 (Manchester) and Samsung deferred. Offline read, no IR hardware (`internal/ir`).
