...
- `jwt_verify` (JWT HMAC signature verify) — ✅ shipped v0.437 — the verification counterpart to `jwt_decode` (decode-only by design). Verifies HS256/384/512 against a candidate secret or a list (the weak-secret test — a top web-pentest primitive), confirms the `alg:none` vulnerability, and reports asymmetric algs as needing a public key (not guessed). Verified in-tree against the canonical jwt.io HS256 token; offline compute, no network/device (`internal/jwtsig`).
